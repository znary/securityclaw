...
import type { RiskScorer } from "../engine/risk_scorer.ts";
...
export function runPolicyGuard(
  ...
  const riskScore = riskScorer.score(context);
  const outcome = decisionEngine.evaluate(context, riskScore, matches);
